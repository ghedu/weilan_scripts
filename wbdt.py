"""
    手动补全token、cookie、sign随便填一个账号的就行
    脚本仅供学习，切勿用于非法用途，请于24小时内删除
    环境变量wbdh="uuid#uuid#uuid"
"""
import random
import time
...
def getQ(mid,uuid,headers,sign):
...
def postQ(uuid,id,headers,sign):
    url = 'https://wbdh.jiiimo.cn/api/question/post'
    data = {
        'id': id,
        'select': questionList[id],
        'wyy': '0',
        'wyy_point': '0',
        'uuid': uuid,
        'sign': sign,
    ...
    response = requests.post(url, headers=headers, data=data).json()
    questionList[id] = list(response['data']['answer'].keys())[0]

def prize(mid,uuid,headers):
    url = 'https://wbdh.jiiimo.cn/api/prize/get'

    data = {
        'mid': mid,
        'uuid': uuid,
        'sign': 'e3d788f84cf7efeee8ecd15d69c151e2c2e074f9',
    }

    response = requests.post(url, headers=headers, data=data).json()
    print(response)

def info(uuid,headers,sign):
    ...
    response = requests.post(url, headers=headers, data=data).json()
    print(response['data']['nickname']+':'+response['data']['balance'])

def tixian(uuid,headers,sign):
...
def run(uuid):
    ...
    for mid in mids:
        for fid in range(20):
            id = getQ(mid,uuid,headers,sign)
            postQ(uuid,id,headers,sign)
            time.sleep(3)
        prize(mid,uuid,headers)
        time.sleep(random.randint(5,10))
    checkin(headers, sign, uuid)
    info(uuid,headers,sign)
    tixian(uuid,headers,sign)

if __name__ == '__main__':
    print('有能力的加群：478552373')
    if os.environ.get("wbdh"):
        tokenString = os.environ["wbdh"]
        if tokenString != '':
    ...
    print('共找到' + str(len(cookies)) + '个账号')
    with ThreadPoolExecutor(max_workers=100) as executor:
        executor.map(run, cookies)