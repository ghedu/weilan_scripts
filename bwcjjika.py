...
#---------简化的框架--------
# 配置参数
tmid = "1755161779339538433"  # 正确答案ID     自己更新 13号的1755161779339538433
            ...
            for question in question_list:
                ...
                for option in option_list:
                    ...
                    if option_id == tmid.strip():
                        ...
                        submit_answer(token, option.get('optionKey'))  # 使用找到的optionKey调用submit_answer函数
...
def submit_answer(token, optionKey):
    """提交答案"""
...
def main():
    ...
    for i, token in enumerate(tokens, start=1):
        ...
        #print(f'------账号 {i}/{total_accounts} -------')
        ttcjjh(token) #天天抽奖判断
        pd_sp_cs(token)#领取碎片判断
        query_exam(token, tmid) #回答问题
        send_sp(token)  # 查询碎片
